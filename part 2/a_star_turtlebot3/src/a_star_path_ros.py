...
gazebo_start_pose=(0,0,0)      # To store the start pose of the robot in gazebo
invert_y=200                   # To invert the y axis

odom_yaw=0.01                  # To store the yaw angle of the robot from odometry
...
def save_map(map):
    ...
    # Publishing the initial state of the robot
    publish_initial_state()
    rospy.sleep(2)


    print("\nTraversing the path:")
    for i,j in zip(range(len(path)),tqdm(range(len(path)))):
...
#-----------------------------------Gazebo-----------------------------------#
            ...
            rpm1,rpm2=nodes[path[i]][3]
            vel_x=0.25*(rpm1+rpm2)* R / 100
            vel_th=(rpm2-rpm1)* R / 23 

            # Publishing the velocity of the robot
            velocity.linear.x = vel_x
            velocity.angular.z = 0 if vel_th==0.0 else vel_th
            pub.publish(velocity)

            # Time for the robot to move
            t=0
            while t<1:
                t+=0.1
                rate.sleep()
...
# Getting user inputs
def get_inputs():
    global start_node,end_node,RPM1,RPM2,animation,gazebo_start_pose
...
# Function to publish the initial state of the robot
def publish_initial_state():
    global gazebo_start_pose
    rospy.wait_for_service('/gazebo/set_model_state')
    # Publishing the initial state of the robot
    set_state=rospy.ServiceProxy('/gazebo/set_model_state',SetModelState)
    state=ModelState()
    state.model_name='burger'
    state.pose.position.x=gazebo_start_pose[0]/100
    state.pose.position.y=gazebo_start_pose[1]/100
    state.pose.position.z=0

    yaw=math.radians(gazebo_start_pose[2])
...
if __name__ == "__main__":
    ...
    path,cost_total=generate_path()   
    
    end_time = time.time()
    
    # Simulating the path in gazebo
    save_map(map)    
    ...
    print("\nTotal cost of the path is: ",cost_total)
    print("\nPath is: ",gazebo_path)
